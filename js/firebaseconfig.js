...
try {
    if (!firebase.apps.length) { 
        // Armazena a instância na window para o pix.js encontrar
        window.firebaseAppBioCode = firebase.initializeApp(firebaseConfig); 
        console.log("Firebase: Conectado ao database-biocode com sucesso! ✨");
    } else {
        window.firebaseAppBioCode = firebase.app();
    }
} catch (error) {
...
const monitorarConexao = () => {
    ...
    console.log("%c DREAMTECH DATABASE ENGINE ", "background: #1e293b; color: #f8fafc; font-weight: bold; padding: 5px;");
